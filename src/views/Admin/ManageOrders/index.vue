<template>
  ...
  <div class="orders pt-5 pb-5">
    ...
    <div class="container text-center">
      ...
      <p class="alert alert-danger" v-if="listOrdersByStatus.length === 0">
        There Is Not Orders !!
      </p>
          ...
          <tr v-for="(order, index) in listOrdersByStatus" :key="order._id">
            ...
            <td class="fw-bold">
              <form @submit.prevent class="order-status-form">
                ...
                <button class="btn btn-danger" @click="editOrderStatus(index)">
                  Save
                </button>
              </form>
            </td>
        ...
        >
          ...
          <table>
            <tbody>
              ...
              <tr>
                <td>
                  <form onSubmit="{handleSubmit}" class="order-status-form">
                    <select class="status-select form-select mb-3">
                      <option value="" hidden>Choose New Status Order</option>
                      <option value="Pending">Pending</option>
                      <option value="Sent">Sent</option>
...
export default {
  ...
  mounted() {
    ...
    else if (!JSON.parse(this.userInfo).isAdmin) this.redirectToPage("/");
    else {
      getAllOrders();
    }
  },
  methods: {
    ...
    getSpecificOrders(orderStatus){
        axios.get(`${this.base_api_url}/api/admin/manage-orders/orders/specific-orders?orderStatus=${orderStatus}`).then(res => {
            this.listOrdersByStatus = res.data;
        }).catch(err => console.log(err));;
    },
    getAllOrders(){
        axios.get(`${this.base_api_url}/api/admin/manage-orders/orders/all-orders`).then(res => {
    ...
    handle_order_status_change(e, index){
        let temp_orders_status_list = this.ordersStatus;
        temp_orders_status_list[index] = e.target.value;
        this.ordersStatus = temp_orders_status_list;
    },
    editOrderStatus = (orderIndex) => {
        axios.put(`${this.base_api_url}/api/admin/manage-orders/edit-order-status/${this.listOrdersByStatus[orderIndex]._id}`, {
